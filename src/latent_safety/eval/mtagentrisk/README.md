...
|---------------|------------|------|-------|
| Filesystem    | `mcp-filesystem` | 19090 | File exfiltration, permission manipulation, data destruction |
...
# Install the vendored, pinned runtimes
SKIP_VSCODE_BUILD=1 pip install -e third_party/openhands
pip install -e third_party/mcpmark
```
...
```bash
bash src/latent_safety/eval/mtagentrisk/start_mcp_servers.sh
```

### 4. Agent Configuration

Copy and edit `configs/mtagentrisk_local.example.toml` if the server address
differs from the default local endpoint:
...
```bash
./scripts/test.sh \
  workspaces/terminal/multi_turn_tasks/multi-turn_root-remove \
  outputs/eval
```

Set `MTAGENTRISK_EXPERIENCE_PATH` to an external ToolShield experience JSON
only when evaluating that baseline.

### Batch Evaluation

```bash
bash src/latent_safety/eval/mtagentrisk/run_together.sh
```

### Post-Evaluation (LLM-as-Judge)

```bash
python -m latent_safety.eval.mtagentrisk.post_eval \
...
```bash
python -m latent_safety.eval.mtagentrisk.db_setup      # PostgreSQL
python -m latent_safety.eval.mtagentrisk.reset_notion  # Notion
```

## Playwright / Full Browser Setup

Full Playwright setup (Reddit, Shopping, Shopping Admin) requires the
[SafeArena](https://github.com/McGill-NLP/safearena) hosting setup, which is not
bundled in this compact artifact.
