...
```bash
bash src/redir/eval/mtagentrisk/start_mcp_servers.sh
```
...
```bash
bash src/redir/eval/mtagentrisk/run_together.sh
```

### Post-Evaluation (LLM-as-Judge)

```bash
python -m redir.eval.mtagentrisk.post_eval \
...
```bash
python -m redir.eval.mtagentrisk.db_setup      # PostgreSQL
python -m redir.eval.mtagentrisk.reset_notion  # Notion
```
